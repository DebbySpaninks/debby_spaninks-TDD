...
const getTotalProfitWithFactors = (crops, environmentFactors) => {
    const getProfitForAllCropsWithFactors = crops.reduce((total, input) => total += ((getRevenueForCrop(input) - getCostsForCrop(input)) * getEnvironmentFactor(input.crop, environmentFactors)), 0);
